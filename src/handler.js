...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (!modeCheck.allowed) {
      console.log(`[DBG] MODE BLOCKED: ${JSON.stringify({isAfk:modeCheck.isAfk,hasJadibots:modeCheck.hasJadibots,isOnlyThisGroup:modeCheck.isOnlyThisGroup})}`);
    ...
    if (debounceMessage(msgKey)) {
      console.log(`[DBG] DEBOUNCE blocked: ${msgKey}`);
      return;
    }
    ...
    if (!plugin.config.isEnabled) {
      console.log(`[DBG] plugin DISABLED: ${m.command}`);
      return;
    }
    ...
    console.log(`[DBG] HANDLER THREW: ${error.message}`);
